    ...
    public class UltraModern5eBusinessLayer : IUltraModern5eBusinessLayer
    {
        ...
        public async Task<Result<string>> GenerateTreasureHoard(int challengeRating)
            => await Safe.ExecuteAsync(async () =>
            {
                ...
                if (treasureHoard.Contains(_shootAndLootTextResult))
                {
                    var shootAndLootResult = await ShootAndLootGenerations(1);
                    if (shootAndLootResult.IsSuccess)
                    {
                        treasureHoard += "<br/>" + ShootAndLootDtoToHtml(shootAndLootResult.Data.First());
                    }
                }
        ...
        private async Task<Result<List<ShootAndLootDto>>> ShootAndLootGenerations(int numberOfShootAndLoots)
            => await Safe.ExecuteAsync(async () =>
            {
                ...
                for (int i = 0; i < numberOfShootAndLoots; i++)
                {
                    ...
                    if (result.IsFailure)
                        return Result<List<ShootAndLootDto>>.Failure(result.Error!, result.ReasonType);
                }
                return Result<List<ShootAndLootDto>>.Success(shootAndLootDtos);
            });

...
using EtlDndMonsterDtos = TWP.Api.Core.DataTransferObjects.ETL.MonsterImageExtractedDataDtos;

namespace TWP.Api.Application.ETL
{
    public class ExtractTransformLoad : IExtractTransformLoad
    {
        private readonly string _folderPath = Path.Combine(Path.GetFullPath(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"..\..\..\..\")), "TWP.Api.Infrastructure", "DndMonsterImages");
        ...
        public async Task<Result> ImportMonsterFromImageAsync(string imagePath)
        {
            return await Safe.ExecuteAsync(async () =>
            {
                ...
                var visionResponse = await _openAiInterops.AnalyzeImageAsync(imageBase64, Dnd5EMonsterImageExtractionPrompt.GetPrompt());

                if (string.IsNullOrEmpty(visionResponse))
                    return Result.Failure("No response from OpenAI Vision API");

                // Parse the JSON response
                var monsterData = JsonSerializer.Deserialize<EtlDndMonsterDtos.MonsterImageData>(
                    visionResponse,
        ...
        private Monster5eDbEntity ConvertToMonsterEntity(EtlDndMonsterDtos.MonsterImageData data)
        {
        ...
        private ActionDbEntity ConvertToActionEntity(EtlDndMonsterDtos.ActionImageData action, Guid monsterId)
        {
